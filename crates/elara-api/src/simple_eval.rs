...
mod tests {
    ...
    #[test]
    fn eval_simple_with_stdlib_executes_os_date_utc_string() {
        ...
        assert_eq!(
            eval_simple_source_with_stdlib(
                SourceId::new(0),
                r#"
                local formatted = os.date("!%Y-%m-%dT%H:%M:%S", 0)
                return string.len(formatted), string.byte(formatted, 1), string.byte(formatted, 11)
                "#,
                &profile,
            ),
            Ok(vec![
                Value::integer(19),
                Value::integer(i64::from(b'1')),
                Value::integer(i64::from(b'T')),
            ])
        );
    }
    ...
    fn eval_simple_with_stdlib_executes_package_searchpath_found() {
        ...
        let values = eval_simple_source_with_stdlib(
            SourceId::new(0),
            "return package.searchpath('Cargo','./?.toml')",
            &profile,
        )
    ...
    #[test]
    fn eval_simple_with_stdlib_executes_package_searchpath_absent() {
        let profile = StdLibProfile::Custom([StdLib::Package].into_iter().collect());

        let values = eval_simple_source_with_stdlib(
            SourceId::new(0),
            "return package.searchpath('zz_no','?')",
            &profile,
        )
        .expect("package.searchpath should evaluate");
    ...
    #[test]
    fn eval_simple_with_stdlib_executes_debug_getmetatable_raw() {
        let profile = StdLibProfile::Custom([StdLib::Base, StdLib::Debug].into_iter().collect());

        assert_eq!(
            eval_simple_source_with_stdlib(
                SourceId::new(0),
                "local t = {}\nlocal mt = { __metatable = 'locked' }\nlocal _ = setmetatable(t, mt)\nreturn rawequal(debug.getmetatable(t), mt), rawequal(getmetatable(t), 'locked')",
                &profile,
            ),
