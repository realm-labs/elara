...
pub use native::{
    DebugHookState, DebugInfoTarget, NativeError, NativeErrorKind, NativeFunctionSpec,
    NativeResult, NativeRuntime, NativeStdFunction,
};
pub use os::OS_NATIVE_FUNCTIONS;
pub use package::{
    PACKAGE_C_ROOT_SEARCHER_NATIVE, PACKAGE_C_SEARCHER_NATIVE, PACKAGE_CONFIG, PACKAGE_CPATH,
    PACKAGE_LUA_SEARCHER_NATIVE, PACKAGE_NATIVE_FUNCTIONS, PACKAGE_PATH,
    PACKAGE_PRELOAD_SEARCHER_NATIVE,
};
...
mod tests {
    ...
    #[test]
    fn io_native_functions_are_discoverable() {
        let functions = native_functions(StdLib::Io);

        assert!(
            functions
                .iter()
                .any(|function| function.descriptor() == FunctionSpec::new(StdLib::Io, "type"))
        );
    }
    ...
    fn os_native_functions_are_discoverable() {
        ...
        assert!(
            functions
                .iter()
                .any(|function| function.descriptor() == FunctionSpec::new(StdLib::Os, "clock"))
        );
        assert!(
            functions
                .iter()
        ...
        assert!(
            functions
                .iter()
                .any(|function| function.descriptor() == FunctionSpec::new(StdLib::Os, "remove"))
        );
        assert!(
            functions
                .iter()
