...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  literal match, match-init, and match-miss helpers,
...
- `tests/conformance` currently contains two hundred ten smoke fixtures across
...
Post-M20.4 literal `string.match` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred ten smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
