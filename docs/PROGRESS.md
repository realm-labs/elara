...
- `tests/conformance` currently contains one hundred seventy-eight smoke fixtures across
...
Post-M20.4 `os.time` default-field fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred seventy-eight smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
