...
- `tests/conformance` currently contains seven hundred fifty-four smoke fixtures across
...
| Conformance | Expanded post-M20.4 | Seven hundred fifty-four smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
