...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  UTF-8 character construction and empty construction, iterator/offset helpers,
  multibyte UTF-8 length/codepoint/offset helpers, empty UTF-8 codepoint
  ranges, missing UTF-8 offset results, multibyte backward UTF-8 offsets,
  bounded UTF-8 length checks, math subtype classification, integral
  float-to-integer conversion, mixed numeric min/max selection, integer/float
...
- `tests/conformance` currently contains one hundred thirty-one smoke fixtures across
...
Post-M20.4 empty `utf8.char` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred thirty-one smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
