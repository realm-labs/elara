...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  construction/extraction, empty byte-string construction, and ranged byte helpers,
  ...
  nil-preserving and empty table packing, empty and default table concatenation,
...
- `tests/conformance` currently contains one hundred sixty-three smoke fixtures across
...
Post-M20.4 default-end `string.sub` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred sixty-three smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
