...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  helpers, base raw traversal and empty traversal helpers, base raw length and empty raw length helpers, base raw table
...
- `tests/conformance` currently contains one hundred fifty-five smoke fixtures across
...
Post-M20.4 empty `next` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred fifty-five smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
