...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  operations, string pattern find-position/capture/backreference/position-capture helpers, plain, escaped-literal, start/end-anchored, balanced, and frontier string-search helpers, string find miss results,
...
- `tests/conformance` currently contains two hundred four smoke fixtures across
...
Post-M20.4 end-anchored `string.find` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred four smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
