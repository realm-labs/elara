...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  empty repeat handling, string byte construction/extraction helpers, math
...
- `tests/conformance` currently contains one hundred four smoke fixtures across
...
Post-M20.4 `string.rep` empty-repeat fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred four smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
