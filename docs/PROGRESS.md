...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  handling, pre-file-handle `io` stubs, package preload/require caching plus
  Lua searcher misses, direct `package.searchpath` misses, unsupported
  `package.loadlib` results, debug frame introspection,
  debug traceback string results, mutable debug registry access, debug upvalue
  inspection/mutation/join behavior, no-hook
...
- `tests/conformance` currently contains forty-four smoke fixtures across
...
Post-M20.4 base `xpcall` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Forty-four smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
