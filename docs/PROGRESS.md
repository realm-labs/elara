...
literal-search `string.find`, basic `%s`, `%c`, `%q`, `%f`, integer-family,
and escaped-percent `string.format`, literal string-replacement `string.gsub`,
...
Completed:
  ...
  - M11.2 executable basic `%s`, `%c`, `%q`, `%f`, integer-family, and escaped-percent string.format native spec.
...
- Add remaining exponential/general/hex float, pointer, width, precision, and
...
M11.2 executable basic `%f` string.format verification passed:
