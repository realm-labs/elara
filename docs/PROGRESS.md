...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  length/codepoint/offset helpers, math subtype classification,
...
- `tests/conformance` currently contains one hundred twelve smoke fixtures across
...
Post-M20.4 `math.type` subtype fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred twelve smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
