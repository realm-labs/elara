...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  backreference match, match-init, match-miss helpers, literal, wildcard, and
  start-anchored substitution,
...
- `tests/conformance` currently contains two hundred twenty-seven smoke fixtures across
...
Post-M20.4 start-anchored `string.gsub` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred twenty-seven smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
