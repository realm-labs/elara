...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  deterministic math functions, deterministic math RNG edge behavior, common
  string transform/slicing helpers, math functions combined with string pattern
  operations, table mutation and default sorting helpers,
...
- `tests/conformance` currently contains fifty-three smoke fixtures across
...
Post-M20.4 `package.config` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty-three smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
