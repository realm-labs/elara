...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  alternate forms, character, float, quoted-string, escaped-percent, and pointer
  output, plus `os.execute` shell availability and success/failure command
...
- `tests/conformance` currently contains one hundred two smoke fixtures across
...
Post-M20.4 `string.format` integer-precision fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred two smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
