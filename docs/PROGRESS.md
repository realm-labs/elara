...
- The conformance standard-library smoke matrix now also includes exact-value
  fixtures for base/table helpers, base raw length helpers, base
  conversion/select helpers, broader deterministic math functions, deterministic
...
- `tests/conformance` currently contains fifty-four smoke fixtures across
...
Post-M20.4 base raw length fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty-four smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
