...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  offset results, multibyte backward UTF-8 offsets, math subtype
...
- `tests/conformance` currently contains one hundred twenty-nine smoke fixtures across
...
Post-M20.4 multibyte backward `utf8.offset` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred twenty-nine smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
