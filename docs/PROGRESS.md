...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  bracket-class, quantifier, and optional-quantifier match, start-anchored match-init, end-anchored match,
...
- `tests/conformance` currently contains two hundred twenty-three smoke fixtures across
...
Post-M20.4 escaped-literal `string.match` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred twenty-three smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
