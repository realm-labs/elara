...
- `tests/conformance` currently contains seven hundred sixty-one smoke fixtures across
...
```bash
cargo fmt -p elara-bytecode -p elara-stdlib -p elara-api -p elara-interp -- --check
cargo test -p elara-bytecode dump_stripped_proto_omits_debug_metadata
...
| Conformance | Expanded post-M20.4 | Seven hundred sixty-five smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
