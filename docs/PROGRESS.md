...
- String pattern preflight now distinguishes malformed Lua patterns from
  ...
  indexes, out-of-range capture references, unmatched close captures, and
  unfinished captures; shared conformance/differential fixtures cover those
...
| Conformance | Expanded post-M20.4 | Five hundred eleven smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
