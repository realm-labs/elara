...
- `tests/conformance` currently contains two hundred forty-one smoke fixtures across
...
Post-M20.4 embedded-NUL `string.reverse` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred forty-one smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
