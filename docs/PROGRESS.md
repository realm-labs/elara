...
M18.1 long-string `table.concat` verification passed:

```bash
cargo fmt --all
cargo test -p elara-stdlib table_concat
cargo test -p elara-api table_concat
cargo fmt --all -- --check
