...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  UTF-8 offsets, bounded UTF-8 length checks, math subtype classification,
...
- `tests/conformance` currently contains one hundred thirty-three smoke fixtures across
...
Post-M20.4 ranged `utf8.codepoint` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred thirty-three smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
