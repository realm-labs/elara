...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  table unpack range and empty-range helpers, and default numeric/string sorting helpers,
...
- `tests/conformance` currently contains one hundred forty-seven smoke fixtures across
...
Post-M20.4 empty-range `table.move` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred forty-seven smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
