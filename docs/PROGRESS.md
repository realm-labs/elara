...
- The base-library conformance and differential matrix now covers `tostring`
  using string-valued table metatable `__name` labels for identity output and
  ignoring non-string `__name` values.
- The base-library conformance and differential matrix now covers `tostring`
  invoking table `__tostring` metamethods, coercing numeric metamethod results,
  rejecting non-stringable results, and preserving scalar metamethod errors.
- The base-library conformance and differential matrix now covers `select`
  coercing numeric string and float positions through Lua-style integer
  conversion while preserving zero-position and bad numeric-string errors.
- The base-library conformance and differential matrix now covers missing
  `assert` conditions producing a protected string error.
- The base-library conformance and differential matrix now covers `assert`
  preserving scalar custom error messages through protected calls.
- `xpcall` now validates the handler argument before invoking the target
  callable, matching Lua 5.5's pre-call handler type check, and the
  conformance matrix covers missing and non-function handler cases.
- The base-library conformance and differential matrix now covers `xpcall`
  missing-target errors being routed through a valid handler.
- The base-library conformance and differential matrix now covers non-callable
  scalar `xpcall` targets being routed through a valid handler.
- `pcall` now validates that a callable argument is present before entering the
  protected call boundary, and the conformance matrix covers nested
  `pcall(pcall)` missing-target behavior.
- The base-library conformance and differential matrix now covers present nil
  `pcall` targets being reported as protected non-callable errors.
- The base-library conformance and differential matrix now covers `pcall`
  invoking table `__call` targets and preserving their multiple results.
- The language conformance and differential matrix now covers direct table
  `__call` dispatch passing the original callee as the first metamethod
  argument.
- The runtime now resolves chained `__call` metamethods until a Lua or native
  callable is reached, preserving Lua's inserted-argument order, and the
  language conformance and differential matrix cover a table-valued `__call`
  chain.
- The base-library conformance and differential matrix now covers `pcall`
  invoking chained table-valued `__call` targets and preserving the protected
  multiple results.
- The base-library conformance and differential matrix now covers `pcall`
  catching scalar errors raised from table `__call` metamethods.
- The base-library conformance and differential matrix now covers `xpcall`
  invoking table `__call` targets after validating the function message
  handler.
- The base-library conformance and differential matrix now covers `xpcall`
  invoking chained table-valued `__call` targets and preserving protected
  multiple results.
- The base-library conformance and differential matrix now covers `xpcall`
  routing scalar errors raised from table `__call` metamethods through the
  message handler.
- The language conformance and differential matrix now covers closures
  observing assignments to captured locals made after closure creation.
- The base-library `error` native now applies Lua-style integer conversion to
  its optional level argument, and conformance covers numeric string, float,
  and invalid string levels.
- The base-library `tonumber` native now applies Lua-style integer conversion
  to the explicit base argument, with conformance covering numeric string,
  float, and invalid string bases.
- The base-library conformance and differential matrix now covers `tonumber`
  missing-value errors.
- The base-library conformance and differential matrix now covers `rawset`
  ignoring trailing arguments after table, key, and value.
- The base-library conformance and differential matrix now covers explicit nil
  keys for `rawget` returning nil.
...
- The math-library conformance and differential matrix now covers
  `math.abs(math.mininteger)` preserving Lua's wrapping integer result.
...
- `string.dump` now honors Lua's truthy strip flag by omitting source, line,
  local, and upvalue debug metadata from Elara internal bytecode dumps; a
  shared fixture covers portable stripped-result shape.
- The string conformance and differential matrix now covers non-boolean truthy
  `string.dump` strip arguments, matching Lua 5.5's `lua_toboolean` handling.
- The string conformance and differential matrix now covers `string.dump`
  rejecting native/C functions while preserving portable error-class results.
...
- The base-library conformance and differential matrix now covers `xpcall`
  passing the thrown error object into the message handler.
- The base-library conformance and differential matrix now covers `pcall`
  preserving nil error objects after a protected failure.
- Protected-call plumbing now preserves Lua error objects raised by standard
  library natives instead of flattening them to strings, so `pcall(error)`,
  `pcall(error, nil)`, and `pcall(assert, false, nil)` return Lua-style nil
  error objects.
- The base-library conformance and differential matrix now covers `xpcall`
  preserving nil error objects raised by the message handler.
- The base-library conformance and differential matrix now covers `pcall`
  preserving non-string scalar error objects after a protected failure.
- The base-library conformance and differential matrix now covers `xpcall`
  passing non-string scalar error objects to the message handler unchanged.
- The base-library conformance and differential matrix now covers `pcall`
  preserving false error objects after a protected failure.
- The base-library conformance and differential matrix now covers `error`
  level zero preserving the string error object without source decoration.
- The base-library conformance and differential matrix now covers `warn`
  control messages returning no Lua values.
- The base-library conformance and differential matrix now covers false
  `__metatable` markers being returned and still protecting the table.

## Remaining Gaps

### Release Conformance Dashboard

- `tests/conformance` currently contains seven hundred sixty-seven smoke fixtures across
...
`cargo fmt -p elara-test -- --check` was also run and still reports
...
| Conformance | Expanded post-M20.4 | Seven hundred sixty-seven smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
