...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  table unpack range and empty-range helpers, and default numeric/string plus
...
- `tests/conformance` currently contains one hundred sixty-five smoke fixtures across
...
Post-M20.4 separator-default `table.concat` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred sixty-five smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
