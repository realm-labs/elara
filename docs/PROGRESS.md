...
M18.1 long-string `math.tointeger` verification passed:

```bash
cargo fmt --all
cargo test -p elara-stdlib math_tointeger
cargo test -p elara-api math_tointeger_accepts_long_strings
cargo fmt --all -- --check
