...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  misses, debug frame introspection, mutable debug registry access, debug
  upvalue inspection/mutation/join behavior, no-hook
...
- `tests/conformance` currently contains thirty-eight smoke fixtures across
...
Post-M20.4 coroutine lifecycle fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Thirty-eight smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
