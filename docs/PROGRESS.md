...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  float-to-integer conversion, and mixed numeric min/max selection,
...
- `tests/conformance` currently contains one hundred fourteen smoke fixtures across
...
Post-M20.4 `math.min`/`math.max` mixed-number fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred fourteen smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
