...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  operations, string pattern find-position/capture/backreference/position-capture helpers, plain, start-anchored, balanced, and frontier string-search helpers, string find miss results,
...
- `tests/conformance` currently contains two hundred two smoke fixtures across
...
Post-M20.4 position-capturing `string.find` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred two smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
