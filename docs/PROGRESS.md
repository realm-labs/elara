...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  UTC `os.time` date-table normalization, and C-locale `os.setlocale`
  behavior.
...
- `tests/conformance` currently contains one hundred seventy-seven smoke fixtures across
...
Post-M20.4 `os.time` normalization fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred seventy-seven smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
