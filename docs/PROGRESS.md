...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  and command status tuples, `os.clock` result classification, `os.tmpname`
  string results, absent `os.getenv` handling,
...
- `tests/conformance` currently contains eighty-nine smoke fixtures across
...
Post-M20.4 `os.execute` status fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Eighty-nine smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
