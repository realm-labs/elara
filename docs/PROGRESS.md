...
The primitive interpreter now dispatches `debug.sethook` call and return hook
callbacks for one-shot native and Lua calls, with Lua-style event strings and
nil line arguments for those events; line/count hook events remain M18.2 work.
...
M18.2 debug hook call/return dispatch validation passed:
...
Continue M18.2 with debug hook line/count event dispatch and JIT hook-state
