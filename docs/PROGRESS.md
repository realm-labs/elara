...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  single-element and comparator sorting helpers,
...
- `tests/conformance` currently contains one hundred eighty-four smoke fixtures across
...
Post-M20.4 `table.sort` comparator fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred eighty-four smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
