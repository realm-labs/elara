...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  misses, debug frame introspection,
...
- `tests/conformance` currently contains thirty-five smoke fixtures across
...
Post-M20.4 final return call-expression and base `pcall` fixture expansion
passed:
...
cargo clippy -p elara-compiler -p elara-interp -p elara-api -p elara-test --all-targets -- -D warnings
git diff --check
```
...
| Conformance | Expanded post-M20.4 | Thirty-five smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
