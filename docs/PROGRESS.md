...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  base conversion/radix/select helpers, base multi-result `select` helpers, base
...
- `tests/conformance` currently contains one hundred thirty-eight smoke fixtures across
...
Post-M20.4 radix `tonumber` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred thirty-eight smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
