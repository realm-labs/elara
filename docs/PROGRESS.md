...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  length/codepoint/offset helpers, empty UTF-8 codepoint ranges, missing UTF-8
  offset results, multibyte backward UTF-8 offsets, bounded UTF-8 length
  checks, math subtype classification, integral
...
- `tests/conformance` currently contains one hundred thirty smoke fixtures across
...
Post-M20.4 bounded `utf8.len` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred thirty smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
