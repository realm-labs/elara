...
M18.2 executable `debug.upvalueid` validation passed:

```bash
cargo fmt --all
cargo test -p elara-api --test debug_upvalueid
cargo test -p elara-stdlib debug_upvalueid
cargo fmt --all -- --check
...
Continue M18.2 with `debug.getlocal`/`debug.setlocal` behavior or
`debug.upvaluejoin`, while keeping coroutine and JIT debug frame behavior
explicit until hook/deopt integration is designed.
