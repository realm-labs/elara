...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  misses, debug frame introspection, debug traceback string results, mutable
...
- `tests/conformance` currently contains thirty-nine smoke fixtures across
...
Post-M20.4 debug traceback fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Thirty-nine smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
