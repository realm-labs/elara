...
Current step: M5.4 Compile constants and arithmetic expressions
...
implemented, along with initial bytecode verification. Compiler, interpreter,
...
```bash
cargo test -p elara-compiler expr_codegen
```

Recommended commit:

```text
feat(compiler): compile arithmetic expressions
```
...
- Compile constants and arithmetic expressions.
...
Implement M5.4 from `docs/MILESTONES.md`:

1. Add a compiler entry point for expression code generation.
2. Compile literals and unary/binary arithmetic expressions.
3. Emit constants and verify generated bytecode.
4. Run `cargo test -p elara-compiler expr_codegen`.
...
| Compiler | Not started | Current milestone. |
