...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  and protected metatable mutation helpers, base raw equality helpers, base raw traversal, after-key traversal, and empty traversal helpers, base raw length and empty raw length helpers, base raw table
  access and nil-clearing helpers, base iteration including `ipairs` nil-stop behavior, and argument-error helpers,
  base conversion/radix/numeric-input/standard-number/select/type helpers, base multi-result, negative-index, and empty-count `select` helpers, base
...
- `tests/conformance` currently contains three hundred forty-six smoke fixtures across
...
| Conformance | Expanded post-M20.4 | Three hundred forty-six smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
