...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  backreference match, match-init, match-miss helpers, literal and wildcard
  substitution,
...
- `tests/conformance` currently contains two hundred twenty-six smoke fixtures across
...
Post-M20.4 dot-wildcard `string.gsub` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred twenty-six smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
