...
- The conformance standard-library smoke matrix now also includes exact-value
  fixtures for base/table helpers, base metatable helpers, base raw length
  helpers, base raw table access helpers, base iteration helpers,
...
- `tests/conformance` currently contains seventy-two smoke fixtures across
...
Post-M20.4 base metatable fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Seventy-two smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
