...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  string width/precision, integer, character, float, quoted-string,
...
- `tests/conformance` currently contains ninety-eight smoke fixtures across
...
Post-M20.4 `string.format` string-modifier fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Ninety-eight smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
