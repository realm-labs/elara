...
- `tests/conformance` currently contains thirty-seven smoke fixtures across
...
Post-M20.4 `os.execute` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Thirty-seven smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
