...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  pre-file-handle `io.type` nil classification, package
...
- `tests/conformance` currently contains ninety-one smoke fixtures across
...
| Conformance | Expanded post-M20.4 | Ninety-one smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
