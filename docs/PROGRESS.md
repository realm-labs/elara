...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  classification, package preload/require caching plus Lua searcher misses,
  direct `package.searchpath` misses, unsupported
...
- `tests/conformance` currently contains fifty-two smoke fixtures across
...
Post-M20.4 `os.remove` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty-two smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
