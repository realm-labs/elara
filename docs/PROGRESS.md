...
- The conformance error smoke matrix now includes parser diagnostics, explicit
  ...
  non-callable values, and arithmetic type errors.
...
- `tests/conformance` currently contains forty-two smoke fixtures across
...
Post-M20.4 `os.getenv` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Forty-two smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
