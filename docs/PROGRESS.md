...
`string.lower`, `string.upper`, `string.reverse`, `string.rep`, `string.packsize`, and
...
- `tests/conformance` currently contains five hundred six smoke fixtures across
...
`string.packsize` behavior and fixture change.
...
| Conformance | Expanded post-M20.4 | Five hundred six smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
