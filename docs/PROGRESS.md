...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  operations, string pattern find-position/capture/backreference/position-capture helpers, wildcard, bracket-class, greedy/optional quantifier, plain, escaped-literal, start/end-anchored, balanced, and frontier string-search helpers, string find miss results,
  match-init and match-miss helpers,
  position-capture helpers, bounded substitution, missing-substitution,
  ...
  explicit, and long-string table concatenation,
  ...
  UTF-8 offsets, long-string UTF-8 operations, empty, bounded, and relative
  UTF-8 length checks, math subtype classification,
  ...
  caching, nil-loader defaults, plus direct preload searcher hits and Lua searcher misses, direct
...
- `tests/conformance` currently contains two hundred eight smoke fixtures across
...
Post-M20.4 optional-quantifier `string.find` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred eight smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
