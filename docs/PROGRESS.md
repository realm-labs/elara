...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  position-capture helpers, capture/replacement helpers, balanced/frontier
  string pattern helpers, table mutation, explicit range/destination helpers,
  table unpack range helpers, and default sorting helpers,
...
- `tests/conformance` currently contains eighty-one smoke fixtures across
...
Post-M20.4 string match init fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Eighty-one smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
