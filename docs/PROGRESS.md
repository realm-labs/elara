...
- `tests/conformance` currently contains five hundred twenty-one smoke fixtures across
...
```bash
cargo test -p elara-stdlib table_integer_arguments_coerce_exact_numbers_and_numeric_strings
cargo test -p elara-stdlib table_concat_bounds_coerce_exact_numbers_and_numeric_strings
cargo test -p elara-stdlib table_
...
table integer-argument coercion change.
...
| Conformance | Expanded post-M20.4 | Five hundred twenty-one smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
