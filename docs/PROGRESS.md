...
- `tests/conformance` currently contains one hundred twenty-six smoke fixtures across
...
Post-M20.4 float `math.fmod` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred twenty-six smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
