...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  access and nil-clearing helpers, base iteration and argument-error helpers,
  ...
  nil-separator, explicit-bound, explicit, and long-string table concatenation,
...
- The conformance language smoke matrix now also includes exact-value fixtures
  ...
  computed table keys,
  negative-step numeric `for` loops,
...
- `tests/conformance` currently contains three hundred thirty-seven smoke fixtures across
...
| Conformance | Expanded post-M20.4 | Three hundred thirty-seven smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
