...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  access helpers, base conversion/select helpers, broader deterministic math
...
- `tests/conformance` currently contains fifty-six smoke fixtures across
...
Post-M20.4 string byte/char fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty-six smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
