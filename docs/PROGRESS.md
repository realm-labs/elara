...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  base conversion/select helpers,
...
- `tests/conformance` currently contains seventy-seven smoke fixtures across
...
Post-M20.4 string position capture fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Seventy-seven smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
