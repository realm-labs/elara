...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  integer output plus `os.execute` shell availability, `os.tmpname` string
  results, absent `os.getenv` handling, absent-file `os.remove`/`os.rename`
  handling, UTC `os.date`/`os.time`, and C-locale `os.setlocale` behavior.
...
- `tests/conformance` currently contains fifty-eight smoke fixtures across
...
Post-M20.4 absent-source `os.rename` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty-eight smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
