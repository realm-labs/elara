...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  results, absent `os.getenv` handling, UTC `os.date`/`os.time`, and C-locale
...
- `tests/conformance` currently contains fifty-one smoke fixtures across
...
Post-M20.4 table sort fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty-one smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
