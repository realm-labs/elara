...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  deterministic math functions, deterministic math RNG edge behavior, math
...
- `tests/conformance` currently contains forty-nine smoke fixtures across
...
Post-M20.4 UTF-8 char fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Forty-nine smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
