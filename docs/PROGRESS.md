...
- The base-library conformance and differential matrix now covers `select`
  ...
  conversion while preserving bad numeric-string errors.
...
- `tests/conformance` currently contains seven hundred fifty-six smoke fixtures across
...
| Conformance | Expanded post-M20.4 | Seven hundred fifty-six smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
