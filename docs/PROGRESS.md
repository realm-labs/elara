...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  operations, table mutation helpers,
...
- `tests/conformance` currently contains fifty smoke fixtures across
...
Post-M20.4 string operations fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
