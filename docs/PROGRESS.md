...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  broader deterministic math functions, deterministic math trig/angle helpers,
  math decomposition helpers, math nil-result classification,
  deterministic math RNG edge behavior, common string transform/slicing helpers, string byte
  construction/extraction helpers, math
  functions combined with string pattern operations, string pattern
  find-position helpers, plain string-search helpers, match-init helpers,
  position-capture helpers, bounded substitution helpers,
  capture/replacement helpers, balanced/frontier string pattern helpers, table
  mutation, explicit range/destination helpers, table unpack range helpers, and
  default sorting helpers,
  UTF-8 character construction and iterator/offset helpers, multibyte UTF-8
  length/codepoint/offset helpers,
  base protected-call multiple returns, base extended protected-call error
  handling, pre-file-handle `io` stubs, pre-file-handle `io.type` nil
  classification, package configuration strings, package preload/require
  caching plus Lua searcher misses, direct `package.searchpath` misses,
  ...
  `debug.gethook`/`debug.sethook` behavior, and deterministic `string.format`
  integer output plus `os.execute` shell availability, `os.clock` result
  classification, `os.tmpname` string results, absent `os.getenv` handling,
  absent-file `os.remove`/`os.rename` handling, UTC `os.date`/`os.time`, and
  C-locale `os.setlocale` behavior.
...
- `tests/conformance` currently contains eighty-two smoke fixtures across
...
Post-M20.4 bounded string substitution fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Eighty-two smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
