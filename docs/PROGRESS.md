...
- `tests/conformance` currently contains sixty-one smoke fixtures across
...
Post-M20.4 debug uservalue fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Sixty-one smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
