...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  base conversion/radix/select/type helpers, base multi-result and empty-count `select` helpers, base
...
- `tests/conformance` currently contains one hundred fifty smoke fixtures across
...
Post-M20.4 empty-count `select` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred fifty smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
