...
M18.1 safe unsupported process-termination `os.exit` verification passed:

```bash
cargo fmt --all
cargo test -p elara-stdlib os_exit
cargo test -p elara-api --test os_exit
