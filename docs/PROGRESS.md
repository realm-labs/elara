...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  access, metatable-bypass access, and nil-clearing helpers, base iteration including `ipairs` nil-stop behavior, and argument-error helpers,
...
- `tests/conformance` currently contains three hundred forty-eight smoke fixtures across
...
| Conformance | Expanded post-M20.4 | Three hundred forty-eight smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
