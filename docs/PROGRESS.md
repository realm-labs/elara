...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  math decomposition helpers and zero-decomposition handling,
...
- `tests/conformance` currently contains one hundred twenty-two smoke fixtures across
...
Post-M20.4 zero-boundary `math.frexp` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred twenty-two smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
