...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  start-anchored, percent-class, bracket-class, and quantifier substitution,
...
- `tests/conformance` currently contains two hundred thirty smoke fixtures across
...
Post-M20.4 quantifier `string.gsub` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred thirty smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
