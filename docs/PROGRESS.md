...
- The conformance standard-library smoke matrix now also includes exact-value
  fixtures for base/table helpers, base raw length helpers, base raw table
  access helpers, base iteration helpers, base conversion/select helpers,
...
- The conformance error smoke matrix now includes parser diagnostics, explicit
  ...
  non-callable values, arithmetic type errors, and debug uservalue type errors.
...
- `tests/conformance` currently contains seventy smoke fixtures across
...
Post-M20.4 table range fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Seventy smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
