...
M18.2 read-only `debug.getupvalue` validation passed:

```bash
cargo fmt --all
cargo test -p elara-stdlib debug_getinfo
cargo test -p elara-api --test debug_getupvalue
cargo test -p elara-stdlib debug_getupvalue
cargo test -p elara-interp native_context_reads_lua_closure_upvalues
...
Continue M18.2 by adding supported `debug.setupvalue`, `debug.upvalueid`, or
