...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  default, positioned, and empty removal, overlapping and explicit range/destination moves,
...
- `tests/conformance` currently contains one hundred forty-six smoke fixtures across
...
Post-M20.4 default string `table.sort` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred forty-six smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
