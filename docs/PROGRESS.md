...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  empty and separator repeat, empty and embedded-NUL reverse, default-end substring, and substring-range handling, string byte
...
- `tests/conformance` currently contains two hundred forty-three smoke fixtures across
...
Post-M20.4 separator `string.rep` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred forty-three smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
