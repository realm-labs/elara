...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  debug local inspection/mutation, debug traceback string results, raw debug
...
- `tests/conformance` currently contains sixty smoke fixtures across
...
Post-M20.4 debug local fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Sixty smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
