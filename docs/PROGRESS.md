...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  find-position helpers, capture/replacement helpers, balanced/frontier string
  pattern helpers, table mutation, explicit range/destination helpers, table
  unpack range helpers, and default sorting helpers,
...
- `tests/conformance` currently contains seventy-six smoke fixtures across
...
Post-M20.4 string find position fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Seventy-six smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
