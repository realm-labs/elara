...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  results, absent `os.getenv` handling, absent-file `os.remove` handling, UTC
...
- `tests/conformance` currently contains fifty-seven smoke fixtures across
...
Post-M20.4 base iteration fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty-seven smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
