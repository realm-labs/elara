...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  length/codepoint/offset helpers, math subtype classification and integral
...
- `tests/conformance` currently contains one hundred thirteen smoke fixtures across
...
Post-M20.4 `math.tointeger` integral-float fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred thirteen smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
