...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  UTF-8 character construction and iterator/offset helpers,
...
- `tests/conformance` currently contains sixty-eight smoke fixtures across
...
Post-M20.4 math nil-result fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Sixty-eight smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
