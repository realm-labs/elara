...
- `tests/conformance` currently contains one hundred fifty-seven smoke fixtures across
...
Post-M20.4 raw equality fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred fifty-seven smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
