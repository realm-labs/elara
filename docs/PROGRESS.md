...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  integer output plus UTC `os.date`/`os.time` and C-locale `os.setlocale`
  behavior.
...
- `tests/conformance` currently contains thirty-six smoke fixtures across
...
Post-M20.4 debug upvalue fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Thirty-six smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
