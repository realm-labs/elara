...
literal-search `string.find`, basic `%s`, `%c`, `%q`, integer-family, and
...
Completed:
  ...
  - M11.2 executable basic `%s`, `%c`, `%q`, integer-family, and escaped-percent string.format native spec.
...
- Add remaining float, pointer, width, precision, and flag handling for
  `string.format`.
...
M11.2 executable basic `%q` string.format verification passed:
