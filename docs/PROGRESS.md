...
semantics. `LEN` executes for
runtime tables with raw array length and `__len` closure fallback. `CALL` can
invoke function-valued `__call` fallback for table operands. `CONCAT` executes
for short strings and can invoke `__concat` closure fallback. The simple
...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  empty and byte-level string case conversion,
  ...
  balanced-delimiter, frontier, backreference, and position-capture
  substitution,
  ...
  invalid-sequence, lax UTF-8 length checks, and lax UTF-8 codepoint results,
  math subtype classification,
...
- The conformance language smoke matrix now also includes exact-value fixtures
  ...
  short-circuit values, and fixed-parameter function
  calls with missing-argument nil fill, fixed parameters combined with named
  vararg tables, and multiple-return local assignment and reassignment.
...
- `tests/conformance` currently contains two hundred seventy-two smoke fixtures across
...
Post-M20.4 logical `and`/`or` lowering passed:

```bash
cargo test -p elara-bytecode verifier
cargo test -p elara-compiler simple_expr_compiles_logical_short_circuit
cargo test -p elara-api eval_simple_returns_logical_operators_from_source
...
cargo clippy -p elara-bytecode -p elara-compiler -p elara-api -p elara-test --all-targets -- -D warnings
git diff --check
```
...
| Compiler | Initial MVP complete | Simple return-expression codegen emits verified bytecode, including unary `not`, logical `and`/`or`, comparison expression lowering, and fixed-result lowering for bare call statements. |
...
| Conformance | Expanded post-M20.4 | Two hundred seventy-two smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
