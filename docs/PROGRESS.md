...
- Conformance fixtures now cover an expanded release smoke matrix across
  ...
  values and selected string-result classes for success fixtures, and an
...
- The conformance standard-library smoke matrix now also includes exact-value
  fixtures for base/table helpers, base assertion helpers, base metatable
  helpers, base raw traversal helpers, base raw length helpers, base raw table
  access helpers, base iteration helpers,
  base conversion/select helpers, base multi-result `select` helpers, base
  scalar `tostring` helpers,
  broader deterministic math functions, integer/float absolute value checks,
  integer-preserving rounding, logarithm identity behavior, zero square-root
  boundary, zero-angle conversion, deterministic math trig/angle helpers,
  math decomposition helpers, integer/fractional split paths, and
  zero-decomposition handling,
  math nil-result classification,
  deterministic math RNG edge behavior, common string transform/slicing helpers,
  empty repeat and substring-range handling, string byte
  construction/extraction helpers, math functions combined with string pattern
  operations, string pattern find-position helpers, plain string-search helpers,
  match-init helpers,
  position-capture helpers, bounded substitution helpers,
  position-capturing iterator helpers, capture/replacement helpers,
  balanced/frontier string pattern helpers, table mutation, append insertion,
  default and empty removal, overlapping and explicit range/destination moves,
  nil-preserving table packing, empty table concatenation, table unpack range
  helpers, and default sorting helpers,
  UTF-8 character construction and iterator/offset helpers, multibyte UTF-8
  length/codepoint/offset helpers, math subtype classification, integral
  float-to-integer conversion, mixed numeric min/max selection, negative
  remainder handling, and unsigned comparison true/false cases,
  base protected-call multiple returns, base extended protected-call error
  ...
  strings, package preload/require
  ...
  pointer output, plus `os.execute` shell availability and success/failure command
...
- The conformance error smoke matrix now includes parser diagnostics, explicit
  ...
  unsupported `string.format` conversion errors.
- The conformance coroutine smoke matrix now also includes wrap, resume success,
  runnable coroutine close, and created-coroutine lifecycle status coverage.

## Remaining Gaps

### Release Conformance Dashboard

- `tests/conformance` currently contains one hundred twenty-three smoke fixtures across
...
Post-M20.4 integer `math.modf` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred twenty-three smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
