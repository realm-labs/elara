...
literal-search `string.find`, basic `%s`, `%c`, integer-family, and
escaped-percent `string.format`, literal string-replacement `string.gsub`,
...
Completed:
  ...
  - M11.2 executable basic `%s`, `%c`, integer-family, and escaped-percent string.format native spec.
...
- Add remaining float, quoted, pointer, width, precision, and flag handling for
  `string.format`.
...
M11.2 executable basic `%c` string.format verification passed:
