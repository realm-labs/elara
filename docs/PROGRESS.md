...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  default and empty removal, explicit range/destination helpers, table unpack range
...
- `tests/conformance` currently contains one hundred eight smoke fixtures across
...
Post-M20.4 `table.remove` default-position fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred eight smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
