...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  capture/replacement helpers, balanced/frontier string pattern helpers,
  table mutation, explicit range/destination helpers, table unpack range
...
- `tests/conformance` currently contains seventy-five smoke fixtures across
...
Post-M20.4 table unpack fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Seventy-five smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
