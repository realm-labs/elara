...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  multibyte UTF-8 length/codepoint/offset helpers, ranged UTF-8 codepoint
  results, empty UTF-8 codepoint
  ranges, missing UTF-8 offset results, multibyte backward and containing
  UTF-8 offsets, bounded and relative UTF-8 length checks, math subtype classification,
...
- `tests/conformance` currently contains one hundred thirty-four smoke fixtures across
...
Post-M20.4 relative-bound `utf8.len` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred thirty-four smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
