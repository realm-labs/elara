...
literal-search `string.find`, basic `%s`, `%c`, `%q`, `%f`, `%e`, `%E`,
integer-family, and escaped-percent `string.format`, literal string-replacement
...
Completed:
  ...
  - M11.2 executable basic `%s`, `%c`, `%q`, `%f`, `%e`, `%E`, integer-family, and escaped-percent string.format native spec.
...
- Add remaining general/hex float, pointer, width, precision, and flag handling
  for `string.format`.
...
M11.2 executable basic `%e`/`%E` string.format verification passed:
