...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  mutation, explicit range/destination helpers, and default sorting helpers,
...
- `tests/conformance` currently contains seventy-four smoke fixtures across
...
Post-M20.4 base `next` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Seventy-four smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
