...
- The conformance language smoke matrix now also includes exact-value fixtures
  ...
  multiple-return local assignment.
...
- `tests/conformance` currently contains two hundred fifty-three smoke fixtures across
...
Post-M20.4 multiple-return local assignment fix passed:

```bash
cargo test -p elara-compiler functions_expand_final_call_for_local_declarations
...
| Conformance | Expanded post-M20.4 | Two hundred fifty-three smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
