...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  integer, character, float, quoted-string, and escaped-percent output, plus `os.execute` shell availability
...
- `tests/conformance` currently contains ninety-six smoke fixtures across
...
Post-M20.4 `string.format` escaped-percent fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Ninety-six smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
