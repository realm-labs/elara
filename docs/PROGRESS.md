...
- `tests/conformance` currently contains six hundred twenty-six smoke fixtures across
...
| Conformance | Expanded post-M20.4 | Six hundred twenty-six smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
