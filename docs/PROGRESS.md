...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  construction/extraction, embedded-NUL length, empty byte-string construction, ranged byte helpers,
...
- `tests/conformance` currently contains two hundred thirty-seven smoke fixtures across
...
Post-M20.4 embedded-NUL `string.len` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred thirty-seven smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
