...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  nil-preserving and empty table packing, empty table concatenation, table unpack range
...
- `tests/conformance` currently contains one hundred forty-four smoke fixtures across
...
Post-M20.4 empty `table.pack` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred forty-four smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
