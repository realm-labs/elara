...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  operations, string pattern find-position helpers, plain string-search helpers, string find miss results,
  match-init and match-miss helpers,
  position-capture helpers, bounded substitution and missing-substitution helpers,
  ...
  `os.date` table/string formatting, deterministic table metadata, and
  weekday/month name aliases, UTC `os.time`, and C-locale `os.setlocale`
  behavior.
...
- `tests/conformance` currently contains one hundred seventy-four smoke fixtures across
...
Post-M20.4 `os.date` name-format fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred seventy-four smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
