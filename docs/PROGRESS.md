...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  offset results, math subtype classification, integral
...
- `tests/conformance` currently contains one hundred twenty-eight smoke fixtures across
...
Post-M20.4 missing-result `utf8.offset` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred twenty-eight smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
