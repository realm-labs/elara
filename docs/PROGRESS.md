...
- `tests/conformance` currently contains eighty-five smoke fixtures across
...
Post-M20.4 UTC date format fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Eighty-five smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
