...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  invalid-sequence, and lax UTF-8 length checks, math subtype classification,
...
- `tests/conformance` currently contains two hundred forty-eight smoke fixtures across
...
Post-M20.4 lax `utf8.len` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred forty-eight smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
