...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  integer and character output, plus `os.execute` shell availability,
  `os.clock` result classification, `os.tmpname` string results, absent
  `os.getenv` handling,
...
- `tests/conformance` currently contains eighty-six smoke fixtures across
...
Post-M20.4 string format character fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Eighty-six smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
