...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  construction/extraction and ranged byte helpers, math functions combined with string pattern
...
- `tests/conformance` currently contains one hundred forty smoke fixtures across
...
Post-M20.4 ranged `string.byte` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred forty smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
