...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  base protected-call multiple returns, pre-file-handle `io` stubs, package
  preload/require caching plus Lua searcher misses, direct `package.searchpath`
  misses, unsupported `package.loadlib` results, debug frame introspection,
...
- `tests/conformance` currently contains forty-three smoke fixtures across
...
Post-M20.4 debug uservalue error fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Forty-three smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
