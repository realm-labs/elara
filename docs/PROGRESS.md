...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  find-position helpers, position-capture helpers, capture/replacement
  helpers, balanced/frontier string pattern helpers, table mutation, explicit
...
- `tests/conformance` currently contains seventy-nine smoke fixtures across
...
Post-M20.4 base multi-result `select` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Seventy-nine smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
