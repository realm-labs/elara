...
- `tests/conformance` currently contains two hundred sixty-two smoke fixtures across
...
| Conformance | Expanded post-M20.4 | Two hundred sixty-two smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
