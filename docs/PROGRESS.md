...
`ceil`, `cos`, `floor`, `max`, `min`, `random`, `sin`, `sqrt`, `tan`, and
`type`. The API layer can build a primitive `RuntimeEnvironment` from
...
M11.2 executable math trig verification passed:
