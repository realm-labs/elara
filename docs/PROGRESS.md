...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  integer output plus `os.execute` shell availability, UTC `os.date`/`os.time`,
...
- `tests/conformance` currently contains forty smoke fixtures across
...
Post-M20.4 `package.loadlib` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Forty smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
