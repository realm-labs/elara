...
- The conformance error smoke matrix now includes parser diagnostics, explicit
  ...
  `os.exit` behavior, plus `coroutine.yield` outside a resume boundary and
  invalid `debug.getinfo` option strings.
...
- `tests/conformance` currently contains two hundred sixty-one smoke fixtures across
...
Post-M20.4 invalid `debug.getinfo` option error fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Two hundred sixty-one smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
