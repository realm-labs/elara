...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  math RNG edge behavior, common string transform/slicing helpers, math
...
- `tests/conformance` currently contains fifty-five smoke fixtures across
...
Post-M20.4 base raw table access fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty-five smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
