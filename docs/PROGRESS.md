...
- `tests/conformance` currently contains five hundred nine smoke fixtures across
...
```bash
/opt/homebrew/bin/lua5.5 -e 'print(dofile("tests/conformance/coroutine/resume_dead.lua"))'
...
| Conformance | Expanded post-M20.4 | Five hundred nine smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
