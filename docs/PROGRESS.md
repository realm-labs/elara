...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  float-to-integer conversion, mixed numeric min/max selection, and negative
  remainder handling,
...
- `tests/conformance` currently contains one hundred fifteen smoke fixtures across
...
Post-M20.4 `math.fmod` negative-dividend fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred fifteen smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
