...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  registry access, and deterministic UTC `os.date`/`os.time` plus C-locale
...
- `tests/conformance` currently contains thirty-one smoke fixtures across
...
Post-M20.4 standard-library debug registry fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Twenty-four smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
