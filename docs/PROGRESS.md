...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  nil-preserving table packing, table unpack range helpers, and default sorting
  helpers,
...
- `tests/conformance` currently contains one hundred ten smoke fixtures across
...
Post-M20.4 `table.pack` nil-preserving fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred ten smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
