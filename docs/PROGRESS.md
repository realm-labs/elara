...
M18.2 JIT debug-hook disable validation passed:

```bash
cargo fmt --all
cargo test -p elara-jit debug_hooks
cargo fmt --all -- --check
cargo clippy -p elara-jit --features jit --all-targets -- -D warnings
...
Continue M18.2 with debug hook installation/callback execution, wiring hook
state to the JIT debug-hook disable switch when hook installation lands, and
keeping unsupported public coroutine-yield paths explicit until those
integration paths are designed.
