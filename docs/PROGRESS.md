...
- `tests/conformance` currently contains five hundred twenty smoke fixtures across
...
```bash
cargo test -p elara-stdlib utf8_integer_arguments_coerce_exact_numbers_and_numeric_strings
...
UTF-8 integer-argument coercion change.
...
| Conformance | Expanded post-M20.4 | Five hundred twenty smoke fixtures run through the public API with exact portable primitive return-value checks and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
