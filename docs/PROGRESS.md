...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  empty repeat, empty reverse, and substring-range handling, string byte
...
- `tests/conformance` currently contains one hundred sixty-two smoke fixtures across
...
Post-M20.4 empty string case-conversion fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred sixty-two smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
