...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  and protected metatable mutation helpers, base raw equality helpers, base raw traversal and empty traversal helpers, base raw length and empty raw length helpers, base raw table
  access and nil-clearing helpers, base iteration helpers,
  base conversion/radix/numeric-input/standard-number/select/type helpers, base multi-result and empty-count `select` helpers, base
  ...
  empty string case conversion,
  empty repeat, empty reverse, default-end substring, and substring-range handling, string byte
  construction/extraction, empty byte-string construction, ranged byte helpers,
  ...
  balanced/frontier string pattern helpers, table mutation, append and positioned insertion,
  default, positioned, and empty removal, overlapping, empty, and explicit range/destination moves,
  nil-preserving and empty table packing, empty, separator-default, and explicit table concatenation,
  table unpack range, default-bound, and empty-range helpers, and default numeric/string plus
  single-element sorting helpers,
  ...
  UTF-8 offsets, empty, bounded, and relative UTF-8 length checks, math subtype classification,
  integral float-to-integer conversion, mixed numeric min/max selection,
  integer/float remainder handling, and unsigned comparison true/false cases,
...
- `tests/conformance` currently contains one hundred sixty-six smoke fixtures across
...
Post-M20.4 default-bound `table.unpack` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred sixty-six smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
