...
`ceil`, `cos`, `deg`, `exp`, `floor`, `fmod`, `log`, `max`, `min`, `modf`,
`rad`, `random`, `sin`, `sqrt`, `tan`, and `type`. The API layer can build a primitive
...
M11.2 executable math fmod/modf verification passed:
