...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  math constants, math subtype nil-results, random result subtypes,
  ...
  `os.setlocale` queries plus supported categories.
...
- `tests/conformance` currently contains one hundred eighty-two smoke fixtures across
...
Post-M20.4 `math.type` nil-result fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | One hundred eighty-two smoke fixtures run through the public API with primitive return-value checks, selected string-result class checks, and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
