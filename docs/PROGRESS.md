...
`%f`/`%e`/`%E`/`%g`/`%G`, `%c`, `%q`, `%p`, and escaped-percent
`string.format`,
...
- Add remaining hex float `%a`/`%A` handling for `string.format`.
...
M11.2 executable string.format non-hex float flag verification passed:
