...
- The conformance standard-library smoke matrix now also includes exact-value
  ...
  broader deterministic math functions, deterministic
  math RNG edge behavior, common string transform/slicing helpers, string byte
  construction/extraction helpers, math
  functions combined with string pattern operations, table mutation and default sorting helpers,
  ...
  caching plus Lua searcher misses, direct `package.searchpath` misses, unsupported
  `package.loadlib` results, debug frame introspection,
  debug traceback string results, raw debug metatable access/mutation, mutable
...
- `tests/conformance` currently contains fifty-nine smoke fixtures across
...
Post-M20.4 `os.clock` fixture expansion passed:
...
| Conformance | Expanded post-M20.4 | Fifty-nine smoke fixtures run through the public API with exact primitive return-value checks for success cases and error-class checks for failure cases; broader API/unit coverage exists, but release-sized conformance remains a product gap. |
